...
단계·권한·대화 세션을 갖고, 백엔드는 앱 창이 닫혀도 업무를 계속 실행한다.
...
- 직원마다 Codex 또는 Claude Code, 모델, 추론 단계와 파일 권한을 선택한다.
- 여러 직원의 진행 설명·추론·명령·도구 사용·응답을 한 피드에서 확인한다.
- 앱을 닫았다 다시 열어도 PostgreSQL 상태와 CLI 세션을 복구한다.
- 파일을 최대 20개까지 첨부하고 이미지 썸네일·생성 이미지·Markdown 결과를 본다.
- 직원별 기록과 전체 보관함을 검색하고 사용량·비용 통계를 확인한다.
...
| 실시간 업무실 | 모든 직원의 진행 이벤트와 최종 응답을 시간순으로 확인 |
| 하단 입력창 | 직원·CLI·모델·추론·권한 선택, 파일 첨부, 업무 실행·중단 |
...
- 진행 설명, 추론, 명령, 도구와 메시지를 구분한 활동 타임라인
- 실행 중 항목과 완료 기록을 분리해 중복 로그를 줄인 표시
- 완료 응답의 타자 효과, 원문 복사와 GitHub Flavored Markdown 렌더링
...
- 실행 당시 CLI·모델·추론 설정과 외부 세션 ID 보존
- 직원별 기록과 전체 대화 보관함
...
2. 백엔드가 선택된 CLI를 작업 폴더에서 실행하고 `turnId`를 반환한다.
3. 진행 이벤트와 응답 초안이 PostgreSQL에 저장되고 `/ws`로 변경 사실이 전달된다.
4. 앱은 `GET /api/live-feed/:turnId`로 최신 상태만 다시 읽는다.
5. `completed`, `failed`, `interrupted` 상태가 되면 최종 결과를 표시한다.
...
마이그레이션을 준비한 뒤 `127.0.0.1:4317`에서 백엔드를 실행한다.
...
| CLI | 모델 선택지 | 추론 단계 |
| --- | --- | --- |
| Codex | 5.6 Sol, 5.6 Terra, 5.6 Luna | `high`, `xhigh`, `max`, `ultra` |
